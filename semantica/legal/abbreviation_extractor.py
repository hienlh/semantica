...
class AbbreviationExtractor:
    ...
    # Pattern 1: "Full Form (ABBREV)" - highest confidence
    # Pattern 2: "Full Form (sau đây gọi tắt là ABBREV)" - high confidence
    # Pattern 3: "Full Form (gọi tắt là ABBREV)" - high confidence
    # Pattern 4: "Full Form (viết tắt là ABBREV)" - high confidence
    ...
    def _extract_full_forms_from_text(self, text: str) -> None:
        ...
        - "Ủy ban nhân dân (sau đây gọi tắt là UBND)"
        """
        for pattern, confidence in self._full_form_patterns:
            for match in pattern.finditer(text):
                ...
                # (not just another abbreviation or number)
    ...
    def _get_full_form(self, abbrev: str) -> Tuple[Optional[str], int]:
        ...
        Returns:
            (full_form, confidence) or (None, 0) if not found
        """
    ...
    def _get_pos_tag_in_context(self, word: str, context: str) -> Optional[str]:
        ...
        Returns:
            POS tag string or None if not found/unavailable
        """
        # Check cache first
        ...
        try:
            ...
            for token, tag in tagged:
                ...
                if token_upper == word_upper:
                    ...
                    return tag
                # Word is part of a compound token (e.g., "CHÍNH" in "CHÍNH PHỦ")
                # If word appears at start of token, use that tag
                if token_upper.startswith(word_upper + " "):
                    self._pos_cache[cache_key] = tag
                    return tag
                # If word appears anywhere in token
    ...
    def _analyze_word(
        ...
        Args:
            word: Uppercase word to analyze
            context: Surrounding text for POS tagging context

        Returns:
            (is_abbreviation, confidence, reason, pos_tag)
        """
        # Remove hyphen for analysis
        clean = word.replace("-", "")
        ...
        # Rule 1: All consonants = definitely abbreviation (HĐQT, TGĐ, VPCP)
        # This is the strongest signal - Vietnamese words always have vowels
        if not has_vowel and len(clean) >= 2:
            return True, 100, "all_consonants", pos_tag

        # Rule 2: Hyphenated + short = likely abbreviation (NĐ-CP pattern)
        if "-" in word and len(clean) <= 6:
            return True, 95, "hyphenated", pos_tag

        # Rule 3: If POS tag indicates common Vietnamese word class, NOT abbreviation
        if pos_tag in VIETNAMESE_WORD_POS_TAGS:
            return False, 10, f"pos_{pos_tag}", pos_tag

        # Rule 4: If has vowels AND tagged as proper noun (Np), likely Vietnamese name
        # Vietnamese words always have vowels, abbreviations often don't
        # Exception: Short words (<=4 chars) with high cons ratio are still abbreviations
        if has_vowel and pos_tag in {"Np", "Ny", "Nb", "Nu"}:
            # Short words with high cons ratio (like UBND) are likely abbreviations
            # Longer words with vowels + Np tag are likely Vietnamese names
        ...
        # This catches abbreviations like UBND (cons_ratio=0.75)
        ...
        # Rule 6: POS = M (classifier) - often used for abbreviations
        if pos_tag == "M":
            # M tag with high consonant ratio = likely abbreviation
            if cons_ratio >= 0.7:
    ...
    def extract_from_text(self, text: str) -> List[AbbreviationMatch]:
        ...
        Returns:
            List of AbbreviationMatch sorted by count descending
        """
        if not text:
            return []
    ...
    def extract_from_texts(self, texts: List[str]) -> List[AbbreviationMatch]:
        ...
        Returns:
            Aggregated list of AbbreviationMatch sorted by count
        """
        ...
        for text in texts:
            ...
            for match in matches:
                if match.abbreviation in aggregated:
                    ...
                    # Update full form if new one has higher confidence
...
def get_full_form(abbrev: str) -> Optional[str]:  # noqa: ARG001
    ...
    This needs to look up from database since we no longer use hardcoded dict.
    Returns None - full forms should be queried from database.
    """
    return None
...
def expand_search_terms(query: str) -> List[str]:
    ...
    Note: Without hardcoded dictionary, this just returns the original query.
    For full expansion, query the database for abbreviations and their full forms.
    """
    return [query]
